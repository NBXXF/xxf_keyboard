...
public class PhoneDouyinLiveActivity extends AppCompatActivity {
    ...
    private Runnable insertMessage = new Runnable() {
        ...
        public void run() {
            if (mAdapter != null) {
                mAdapter.insertMessage(new Message("XXF", "我来啦 " + mAdapter.getItemCount()));
            }
    ...
    private void initView() {
        ...
        mBinding.send.setOnClickListener(v -> {
            mAdapter.insertMessage(new Message("XXF", mBinding.input.getText().toString()));
        ...
        for (int i = 0; i < 100; i++) {
            messages.addFirst(new Message("XXF" + i, "唱的好好听哦"));
        }
